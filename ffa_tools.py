...
def cands_to_file(cands,name):
	...
	"""
	k=int(1)
	fo = open(name+'_precands.ffa','a')
	for i in range(len(cands.periods)):
		fo.write(str(k)+'\t'+'\t'+\
		str(cands.periods[i])+'\t'+'\t'+str(cands.SNRs[i])\
		+'\t'+'\t'+str(cands.dts[i])+'\n')
		k+=1
	print "Wrote ", str(k), "candidates in : "+name+'_precands.ffa'
	write_inf(name,fo)
	fo.close()

def pick_cands(ps, sns, dts, SN_tresh, write_cands, name):
	"""
...
def apply_sifting(candsfile_list, output_name):
	"""
	...
	"""
	cands = sf.read_candidates(candsfile_list)
	print 'read cands done'
