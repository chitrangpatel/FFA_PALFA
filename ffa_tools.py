...
def pick_cands(ps, sns, dts, SN_tresh, write_cands, name,suffix):
	"""
	Select candidates periods that have a period whose S/N is above 
	the S/N treshold, and then adds those periods to the object cands (ffa_cands())
	Inputs:
		ps		: list of periods
		sns		: list of S/N ratios
		dts		: list of samplings interval
		SN_tresh	: float, limit above which periods are selected
		write_cands	: False until you reach the highest duty cycle for the beam 
		name		: Name of the beam
	Returns : None
	Writes candidates to cands()	
 			
	"""	
	j = sns >= SN_tresh
	if np.array(j).any():
		for i in range(len( np.concatenate(np.where(j)))):
			cands.add_cand(round(np.array(ps)[j][i],4),\
			round(sns[j][i],1), round(np.array(dts)[j][i],5))

	if write_cands:
		cands_to_file(cands,name,suffix)
	return cands


def format_final_cands(filenm):
	"""
	To be called once the DM sifting has been applied. 
	It does basic formatting, and sorts the candidates according to S/N.
	Also writes the p/P ratio, where P = period of the best candidate.
	Inputs:
		filenm: str, name of the file containing sifted candidates (beam+"_cands.ffa")
	Returns: None
	...
	for lin in range(len(lines)):
    		lines[lin][3] = float(lines[lin][3])

	lines.sort(key=itemgetter(3),reverse=True)
	
	for l in range(len(lines)):
		del lines[l][1]
		lines[l][0] = re.sub('\:$',"",lines[l][0])
		lines[l][1] = '\t'+ '\t'+lines[l][1]
		lines[l][2] = '\t'+ '\t'+str(lines[l][2])
		lines[l][3] = '\t'+ '\t'+lines[l][3]
		lines[l][4] = '\t'+'\t'+lines[l][4]
		lines[l].append('\t'+'\t'+str(round(float(lines[l][1])/float(lines[0][1]),3)))
...
def format_final_cands2(filenm):
	"""
	To be called once the DM sifting has been applied. 
	It does basic formatting, and sorts the candidates according to S/N.
	Also writes the p/P ratio, where P = period of the best candidate.
	...
	with open(filenm+'_test', 'w') as fout:
		fout.write("# filename"+'\t'+'\t'+'\t'+'\t'+'\t'+'\t'+'\t'+\
			"P(ms)"+'\t'+'\t'+"S/N"+\
			'\t'+ '\t'+"DM"+'\t'+'\t'+"dt(ms)"+'\t'+'\t'+"p/P"+'\n')
		#fout.write("# filename"+'\t'+'\t'+'\t'+'\t'+'\t'+'\t'+'\t'+\
...
def apply_sifting(candsfile, output_name):
	...
	"""
	cands = sf.read_candidates(candsfile)
	cands.remove_duplicate_candidates()
	cands.remove_harmonics()
	print 'Wrote ',len(cands),' final cands in ', output_name
	cands.to_file(candfilenm = output_name)


def final_candfile(name_beam, list_candfiles, output_file):
	#name_beam is, for example, J1851+0242_dm650   -> no .dat, no .inf
	fin = open(list_candfiles,'r')
	...
	for fil in files:
		...
		for l in lines_temp:
			print l
			string = l[0].center(20)+"	"+l[1].center(40) +l[2].center(2) +l[3].center(24) + l[4].center(24) +'\n'
			fout.write(string)
	write_inf(re.sub('\.dat$',"",name_beam),fout,write_DM=False)
	fout.close()
	...
	ftemp.close()
	candidates = sf.read_candidates(candfile_for_sifting)
	candidates.remove_duplicate_candidates()
	candidates.remove_harmonics()
	#candidates.remove_DM_problems()
